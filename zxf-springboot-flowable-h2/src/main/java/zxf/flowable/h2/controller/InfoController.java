...
public class InfoController {
    ...
    public List<String> executionsByMessage(@RequestParam String message) {
        ...
        List<Execution> instances = processEngine.getRuntimeService().createExecutionQuery()
                .messageEventSubscriptionName(message).list();
        return instances.stream().map(flowableService::executionInfo).collect(Collectors.toList());
    }
    ...
    public List<String> allTasks() {
        log.info("allTasks");
        List<Task> tasks = processEngine.getTaskService().createTaskQuery().processDefinitionKey("LoanProcess").list();
    ...
    public List<String> registeredDeployments() {
        ...
        // Flowable 无 deployment-aware，返回全部流程定义。
